...
public class UpdateMoonPage {
	public static Composite updateMoonPage(Composite parent, Connection conn,
		...
		// Here is where we fetch the data for the planet and populate the
		// widgets with the data from the DB
		// This part looks really similar to the insert planet page
		// For this page it is really important to pass in the planet selection
		// Combo so that we can refresh it
		// after the planet is updated in case they change the name. Do this to
		// avoid sending updates to the SQL
		...
		updateGD = new GridData(SWT.FILL, SWT.CENTER, false, false);
		Text galaxySelect = new Text(moonUpdatePage, SWT.READ_ONLY);
		updateGD.horizontalSpan = 2;
		...
		try {
			...
			while (rs.next()) {
				for (int i = 1; i <= rsmd.getColumnCount(); i++) {
					String columnValue = rs.getString(i);
					galaxySelect.setText(WordUtils.capitalize(columnValue));
				}
			}
		...
		// ////////////////From here on down it gets a little
		// messy//////////////////////////////
		submit.addListener(SWT.Selection, new Listener() {
			public void handleEvent(Event event) {
				...
				if (!error) {
					// Now that all the data is good send UPDATE statement to
					// the sever
					try {
						// It's really important to turn off the check for
						// foreign keys
						// If you don't, you will get an exception when
						// executing the statement
						// We do this so if they decide to change what star it
						// orbits or galaxy it's in
				...
				// Now we want to refesh the planets drop-down with the new name
				// (if they changed it)
				// We just tell the refresh button that it has been pushed, when
				// it really hasn't
			...
			public void widgetSelected(SelectionEvent e) {
				try {
					PreparedStatement getGalaxyName = conn
							.prepareStatement("SELECT inGalaxy FROM planet WHERE name='"
									+ planetSelect.getText() + "';");
		...
		refresh.addListener(SWT.Selection, new Listener() {
			public void handleEvent(Event event) {
				try {
					// Same as other refresh buttons, just force it to do an
					// extra few steps
					PreparedStatement getPlanetNames = conn
							.prepareStatement("SELECT DISTINCT name FROM planet ORDER BY name;");
					getPlanetNames.execute();
				...
				// Here we just refresh the input boxes with the updated data
				// from the DB
				try {
					...
					getMoonValues(conn, name, nameBox, massBox, periodBox,
							planetSelect, galaxySelect);
	...
	public static void getMoonValues(Connection conn, String moonName,
			Text nameBox, Text massBox, Text periodBox, Combo planetSelect,
			Text galaxySelect) {
