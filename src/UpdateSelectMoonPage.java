...
public class UpdateSelectMoonPage {
	public static Composite createSelectUpdatePage(Composite parent, Connection conn) {
		...
		Label moonUpdateName = new Label(updatePage, SWT.NONE);
		updateGD.horizontalSpan = 4;
		moonUpdateName.setText("Select a Moon to update:");
		moonUpdateName.setLayoutData(updateGD);

		updateGD = new GridData(SWT.FILL, SWT.CENTER, false, false);
		Combo updateMoonSelect = new Combo(updatePage, SWT.DROP_DOWN
				| SWT.READ_ONLY);
		updateGD.horizontalSpan = 2;
		updateMoonSelect.setLayoutData(updateGD);
		...
		try {
			...
			while (rs.next()) {
				for (int i = 1; i <= rsmd.getColumnCount(); i++) {
					...
					updateMoonSelect.add(WordUtils.capitalize(columnValue));
				}
			}
		...
		updateMoonSelect.addListener(SWT.Selection, new Listener() {
			@Override
			public void handleEvent(Event e) {
				Composite moonPage = UpdateMoonPage.updateMoonPage(sharedComposite, conn, updateMoonSelect.getText(), updateMoonSelect);
